// Add global handlers for login/signup buttons
function handleLogin() {
  ...
  console.log('Login handler running with username:', username);
  ...
  // Use our safer socket method
  const success = safeSocketEmit('login', { username, password });
...
function handleSignup() {
  ...
  console.log('Signup handler running with username:', username);
  ...
  // Use our safer socket method
  const success = safeSocketEmit('signup', { username, password });
...
document.addEventListener('DOMContentLoaded', () => {
  // Detect Safari browser specifically and add special class
  if (isSafari()) {
    ...
    style.textContent = `
      ...
      .safari-browser .login-tab {
        cursor: pointer !important;
      }
      
      /* Explicitly fix the chat interface layout for iPad Safari */
      .safari-browser body {
    ...
    // Special fix for iPad Safari display
    if (isIOSDevice()) {
      console.log('iPad/iOS device detected - applying special UI fixes');
      
      // Force the initial layout calculation
      setTimeout(() => {
        const loginScreen = document.getElementById('login-screen');
    ...
    // Direct event handlers for login and signup buttons
    const loginBtn = document.getElementById('login-button');
    const signupBtn = document.getElementById('signup-button');
    
    if (loginBtn) {
      loginBtn.onclick = function(e) {
        console.log("Safari login button clicked");
        const username = document.getElementById('username-input').value.trim();
        const password = document.getElementById('password-input').value.trim();
    ...
    if (signupBtn) {
      signupBtn.onclick = function(e) {
        console.log("Safari signup button clicked");
        const username = document.getElementById('signup-username-input').value.trim();
        const password = document.getElementById('signup-password-input').value.trim();
        const confirmPassword = document.getElementById('signup-confirm-password').value.trim();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('button').forEach(button => {
    button.addEventListener('touchstart', function(e) {
      // Don't prevent default for Safari compatibility
      // This allows the natural click to happen afterward
  ...
  // Login button - use a single unified handler for all events
  if (loginButton) {
    ...
    // Safari-specific handler that doesn't interfere with click events
    if (isIOSDevice()) {
      loginButton.addEventListener('touchend', function(e) {
        // Only handle if this is actually Safari
  ...
  // Signup button - use a single unified handler for all events
  if (signupButton) {
    ...
    // Safari-specific handler that doesn't interfere with click events
    if (isIOSDevice()) {
      signupButton.addEventListener('touchend', function(e) {
        // Only handle if this is actually Safari
        if (/safari/i.test(navigator.userAgent) && !/chrome/i.test(navigator.userAgent)) {
          handleSignup();
        }
...
socket.on('login success', (data) => {
  ...
  // Enhanced visibility handling for iOS Safari
  loginScreen.style.display = 'none';
  
  // Force layout recalculation for Safari
  document.body.classList.add('chat-active');
  
  // Additional fixes for Safari and iOS
  if (isSafari() || isIOSDevice()) {
    console.log('Applying Safari/iOS specific fixes for chat display');
    document.body.classList.add('ios-device');
    
    // Force visible display of chat components with higher specificity
    const chatArea = document.getElementById('chat-area');
    const sidebar = document.getElementById('sidebar');
    ...
    if (sidebar) {
      ...
      sidebar.style.visibility = 'visible';
      sidebar.style.opacity = '1';
      // On mobile, make sidebar active by default
    ...
    setTimeout(() => {
      window.scrollTo(0, 0);
      if (chatArea) chatArea.style.zIndex = chatArea.style.zIndex;
      if (sidebar) sidebar.style.zIndex = sidebar.style.zIndex;
    }, 50);
  }
...
function isIOSDevice() {
  ...
  return /ipad|iphone|ipod|mac|safari/.test(userAgent) && !window.MSStream;
}
...
function makeButtonIOSFriendly(buttonElement) {
  ...
  buttonElement.addEventListener('touchstart', function(e) {
    // Don't prevent default here to allow better Safari compatibility
  ...
  buttonElement.addEventListener('touchend', function(e) {
    ...
    // Don't prevent default or simulate click here
    // The natural click event will fire on Safari
  }, false);
  
  // Also handle touchcancel
  buttonElement.addEventListener('touchcancel', function(e) {
    this.classList.remove('ios-active');
  }, false);
...
// Add Safari-specific handlers when DOM is loaded
document.addEventListener('DOMContentLoaded', function() {
  // If it's Safari, add direct touch handlers for login and signup buttons
  if (isSafari() || /ipad|iphone|ipod|mac/.test(navigator.userAgent.toLowerCase())) {
    console.log('Safari detected, adding direct touch handlers');
    ...
    if (loginButton) {
      loginButton.addEventListener('touchend', function(e) {
        console.log('Login touchend event fired');
        handleLogin();
      }, false);
    }
    ...
    if (signupButton) {
      signupButton.addEventListener('touchend', function(e) {
        console.log('Signup touchend event fired');
        handleSignup();
      }, false);
    }
    
    // Also fix the login/signup tabs for Safari
    const loginTab = document.querySelector('.login-tab[data-tab="login"]');
    const signupTab = document.querySelector('.login-tab[data-tab="signup"]');
    
    if (loginTab) {
      loginTab.addEventListener('touchend', function(e) {
        console.log('Login tab touchend fired');
        this.classList.add('active');
        signupTab.classList.remove('active');
    ...
    if (signupTab) {
      signupTab.addEventListener('touchend', function(e) {
        console.log('Signup tab touchend fired');
        this.classList.add('active');
        loginTab.classList.remove('active');
...
function isSafari() {
  const userAgent = navigator.userAgent.toLowerCase();
  return /safari/.test(userAgent) && !/chrome/.test(userAgent) && !/firefox/.test(userAgent) && !/edge/.test(userAgent);
}

// Safety function to ensure socket works in Safari
function safeSocketEmit(eventName, data) {
  console.log(`Safe socket emit: ${eventName}`, data);
  
  // Try with global socket first
  try {
    if (typeof socket !== 'undefined' && socket) {
      socket.emit(eventName, data);
      console.log(`Emitted ${eventName} with global socket`);
      return true;
    }
  } catch (e) {
    console.error('Error with global socket:', e);
  }
  
  // Try with new socket connection as fallback
  try {
    const fallbackSocket = io();
    fallbackSocket.emit(eventName, data);
    console.log(`Emitted ${eventName} with fallback socket`);
    return true;
  } catch (e) {
    console.error('Error with fallback socket:', e);
    return false;
  }
}

// Safari-specific fixes
if (/safari/i.test(navigator.userAgent) && !/chrome|firefox|edg/i.test(navigator.userAgent)) {
  document.addEventListener('DOMContentLoaded', function() {
    console.log('⚠️ Safari detected! Adding special handlers!');
    
    // Make isSafari globally available for the HTML onclick attributes
    window.isSafari = function() { return true; };
    
    // Direct login and signup handlers for Safari
    window.handleLogin = function() {
      console.log('Safari direct login handler called');
      const username = document.getElementById('username-input').value.trim();
      const password = document.getElementById('password-input').value.trim();
      ...
      // Try direct Socket.IO connection
      try {
        // Create a fresh socket connection
        const directSocket = io();
        
        // Add event listeners first
        directSocket.on('login success', function(data) {
          console.log('Login success event received in Safari:', data);
          // Hide login screen
          ...
          if (data && data.username) {
            ...
            if (data.authentificator) {
              window.currentUserAuthenticator = data.authentificator;
            }
          }
          
          // Load users
          directSocket.emit('load users');
        });
        
        directSocket.on('login failed', function(message) {
          alert(message || 'Login failed');
        });
        
        // Then emit the login event
        console.log('Safari: Emitting direct login with username:', username);
        directSocket.emit('login', { username, password });
        
      } catch (err) {
        console.error('Safari login error:', err);
        alert('Error during login. Please try again.');
      }
      
      // Do NOT submit the form - prevent the default form submission
      return false;
    };
    
    window.handleSignup = function() {
      console.log('Safari direct signup handler called');
      const username = document.getElementById('signup-username-input').value.trim();
      const password = document.getElementById('signup-password-input').value.trim();
      const confirmPassword = document.getElementById('signup-confirm-password').value.trim();
      
      console.log('Safari signup form data:', { username, hasPassword: !!password, hasConfirmPwd: !!confirmPassword });
      ...
      try {
        ...
        // Add event listeners first
        directSocket.on('signup successful', function(username) {
          console.log('Signup success in Safari:', username);
          alert('Signup successful! You are now logged in.');
          // Hide login screen
          document.getElementById('login-screen').style.display = 'none';
        ...
        directSocket.on('signup failed', function(message) {
          alert(message || 'Signup failed');
        });
        
        // Then emit the signup event
        console.log('Safari: Emitting direct signup with username:', username);
        directSocket.emit('signup', { username, password });
        
      } catch (err) {
        console.error('Safari signup error:', err);
    ...
    // Add additional tab handling for iOS/Safari
    window.showLoginTab = function() {
      console.log('Safari: Activating login tab');
      document.querySelector('.login-tab[data-tab="login"]').classList.add('active');
      document.querySelector('.login-tab[data-tab="signup"]').classList.remove('active');
      document.getElementById('login-form').style.display = 'block';
      document.getElementById('signup-form').style.display = 'none';
    };
    
    window.showSignupTab = function() {
      console.log('Safari: Activating signup tab');
      document.querySelector('.login-tab[data-tab="signup"]').classList.add('active');
      document.querySelector('.login-tab[data-tab="login"]').classList.remove('active');
      document.getElementById('signup-form').style.display = 'block';
      document.getElementById('login-form').style.display = 'none';
    };
  });
...
// Add direct touch event handling for tabs
document.addEventListener('DOMContentLoaded', function() {
  console.log('Adding touch event handlers for login/signup tabs');
  ...
  if (loginTab && signupTab) {
    // Touch event for login tab
    loginTab.addEventListener('touchend', function(e) {
      console.log('Login tab touchend');
      e.preventDefault();
      this.classList.add('active');
      signupTab.classList.remove('active');
      loginForm.style.display = 'block';
    ...
    // Touch event for signup tab
    signupTab.addEventListener('touchend', function(e) {
      console.log('Signup tab touchend');
      e.preventDefault();
      this.classList.add('active');
      loginTab.classList.remove('active');
      signupForm.style.display = 'block';
    ...
    // Enhanced mouse click events too
    loginTab.addEventListener('mousedown', function(e) {
      console.log('Login tab mousedown');
      this.classList.add('active');
      signupTab.classList.remove('active');
      loginForm.style.display = 'block';
      signupForm.style.display = 'none';
    ...
    signupTab.addEventListener('mousedown', function(e) {
      console.log('Signup tab mousedown');
      this.classList.add('active');
      loginTab.classList.remove('active');
      signupForm.style.display = 'block';
      loginForm.style.display = 'none';
